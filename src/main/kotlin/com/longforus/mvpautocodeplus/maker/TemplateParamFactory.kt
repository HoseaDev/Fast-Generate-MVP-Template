...
//                <I${NAME}Contract.View,I${NAME}Contract.Presenter>
    ...
    fun getNameAndGenericType(type: String, isContract: Boolean = true, enterName: String = "", selectedValue: String = ""): Pair<String?, String> {
        ...
        if (indexOf > -1) {
            ...
            g.forEach {
                when (it) {
                    "V"[0] -> sb.append(if (isContract) "View" else "${getContractName(enterName)}.IView")
                    "P"[0] -> sb.append(if (isContract) "Presenter" else "${getContractName(enterName)}.IPresenter")
                    "M"[0] -> sb.append(if (isContract) "Model" else "${getContractName(enterName)}.IModel")
                    else -> sb.append(it)
                }
            }
            generic = sb.toString()
